  ...
  return (
    <div className="top-0 fixed w-[100%] z-10">
      <div className="flex justify-between items-center bg-slate-800 py-5 px-3 md:px-10 text-white">
        <div className="relative w-[210px] h-[50px]">
          <Image
          ...
          <ul className="flex flex-row gap-6">
            {headerList.map((res) => (
              <li key={res.id}>
                <Link
                  href={res.link}
                  className="flex items-center text-[#fff] hover:border-b-2 hover:text-[#fff] hover:border-gray-300 gap-1 cursor-pointer"
              ...
              <ul className="flex flex-col gap-3 mt-4 h-[100vh]">
                {headerList.map((res) => (
                  <li key={res.id} className="relative px-3">
                    <Link
                      href={res.link}
                      className="flex items-center gap-1 cursor-pointer text-[#fff]"
                      onClick={() => handleLinkClick(res.name)}
                    >
                      {res.name}
                    </Link>
